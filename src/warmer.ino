...
PID* controller = new PID(&input, &output, &setPoint, 25, 0.5, 0, DIRECT);;
...
//      Serial.println("C");
    ...
    Serial.print(input);
    if (error >= 0){
        controller->SetOutputLimits(30, setPoint*2.3);
    } else {
      /*if (error > -5 && error <= -2) {
      ...
      if (error >-1 && error < -0.2) {
        //Serial.println("controlling1");
        controller->SetTunings(5, 1.5, 0);
        controller->SetOutputLimits(30, 140);
      }
      if (error >= -0.2 && error < 0){
        controller->SetTunings(20, 4, 0);
        controller->SetOutputLimits(35, setPoint*2.5+(setPoint-36)*5);
      }
    }
    controller->Compute();
    //controller->Compute();
